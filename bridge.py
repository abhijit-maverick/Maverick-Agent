    ...
    if m:
        ...
        for c in data.get("candidates", []):
            rows.append({
                ...
                "technical_read": {
                    "structure": "MOCK: structure intact",
                    "relative_strength": "MOCK: favourable",
                    "directional_progress": "MOCK: progress present",
                    "chase_risk": "MEDIUM",
                    "sector_context": "UNKNOWN",
                },
                "risk_checks": {
                    "event": {"status": "UNKNOWN", "detail": "MOCK MODE"},
                    "fno_ban": {"status": "UNKNOWN", "detail": "MOCK MODE"},
                    "mwpl": {"status": "UNKNOWN", "detail": "MOCK MODE"},
                    "expiry": {"status": "UNKNOWN", "detail": "MOCK MODE"},
                    "corporate_action": {"status": "UNKNOWN", "detail": "MOCK MODE"},
                    "surveillance": {"status": "UNKNOWN", "detail": "MOCK MODE"},
                },
...
def _risk_item(value) -> dict:
    if isinstance(value, dict):
        status = str(value.get("status", "UNKNOWN")).upper().strip() or "UNKNOWN"
        detail = str(value.get("detail", ""))
    else:
        status = "UNKNOWN"
        detail = str(value or "NOT SUPPLIED")
    if status not in {"CLEAR", "RISK", "UNKNOWN", "LOW", "MEDIUM", "HIGH", "OK", "BLOCK"}:
        status = "UNKNOWN"
...
def normalize_batch_result(obj: dict, expected_symbols: list[str], batch_number: int, total_batches: int) -> dict:
    ...
    for row in rows:
        ...
        tech_raw = row.get("technical_read") if isinstance(row.get("technical_read"), dict) else {}
        technical_read = {
            "structure": str(tech_raw.get("structure", "NOT SUPPLIED")),
            "relative_strength": str(tech_raw.get("relative_strength", "NOT SUPPLIED")),
            "directional_progress": str(tech_raw.get("directional_progress", "NOT SUPPLIED")),
            "chase_risk": str(tech_raw.get("chase_risk", "NOT SUPPLIED")).upper(),
            "sector_context": str(tech_raw.get("sector_context", "UNKNOWN")),
        }
        rr = row.get("risk_checks") if isinstance(row.get("risk_checks"), dict) else {}
        risk_checks = {
            "event": _risk_item(rr.get("event")),
            "fno_ban": _risk_item(rr.get("fno_ban")),
            "mwpl": _risk_item(rr.get("mwpl")),
            "expiry": _risk_item(rr.get("expiry")),
            "corporate_action": _risk_item(rr.get("corporate_action")),
            "surveillance": _risk_item(rr.get("surveillance")),
        }
        by_symbol[sym] = {
            "symbol": sym,
            "verdict": verdict,
    ...
    if missing:
        raise RuntimeError("Agent omitted stocks from the underwriting response: " + ", ".join(missing))
    return {
        "schema_version": "1.1",
        "type": "maverick_batch_result",
...
def make_batch_prompt(req: dict) -> str:
    ...
    schema = {
        ...
        "results": [{
            ...
            "technical_read": {
                "structure": "brief read of 1h/4h structural state",
                "relative_strength": "brief relative strength/weakness read",
                "directional_progress": "whether the existing trend is still making progress now",
                "chase_risk": "LOW|MEDIUM|HIGH",
                "sector_context": "verified sector context or UNKNOWN"
            },
            "risk_checks": {
                "event": {"status": "CLEAR|RISK|UNKNOWN", "detail": "results/board meeting/business update detail"},
                "fno_ban": {"status": "CLEAR|RISK|UNKNOWN", "detail": "ban check"},
                "mwpl": {"status": "CLEAR|RISK|UNKNOWN", "detail": "utilisation/trend check"},
                "expiry": {"status": "CLEAR|RISK|UNKNOWN", "detail": "near-contract expiry and sessions remaining"},
                "corporate_action": {"status": "CLEAR|RISK|UNKNOWN", "detail": "buyback/dividend/promoter/block-deal/material action"},
                "surveillance": {"status": "CLEAR|RISK|UNKNOWN", "detail": "ASM/GSM/trade-to-trade/regulatory surveillance"}
            },
    ...
    return (
        ...
        + "\n\n## LOCAL AGENT EXECUTION CONTRACT — ONE STOCK AT A TIME\n"
        + "You are processing one Maverick stock. Judge only the supplied symbol. Do not forecast future price. "
          "First prove whether the trend already exists and remains intact; then assess entry readiness and external risk. "
          "Fill every risk_checks field explicitly. If you have a browser/web tool, use it for fresh external checks. "
          "If you cannot verify a live field, mark that field UNKNOWN and apply the strategy's WATCH cap. Never treat missing research as CLEAR.\n"
...
def build_final_prompt(req: dict, session: dict) -> str:
    ...
    return (
        ...
        + "All individual stock underwriting is complete. Compare the surviving TAKE/WATCH names globally. "
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path == "/api/underwrite":
                print(f"\n>>> Session {data.get('session_id')} · Stock {data.get('batch_number')}/{data.get('total_batches')} · agent={AGENT}", flush=True)
                result = process_underwrite(data)
                print(f"<<< Stock {data.get('batch_number')} complete" + (" · FINAL READY" if result.get("final_result") else ""), flush=True)
